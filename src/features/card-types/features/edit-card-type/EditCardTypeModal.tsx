...
import EditCardTypeController from "./EditCardTypeController";
import {useMemo} from "react";
import FieldUtils from "../../../../utils/FieldUtils";

interface EditCardTypeModalProps {
    controller: EditCardTypeController
}


export default function ({controller}: EditCardTypeModalProps) {
    ...
    const data = useMemo(() => {
        ...
        return {
            cardType: cardType,
            fields: FieldUtils.getInstance().getAllBy("cardTypeId", cardType.id)
        }


    }, [controller.states.showState.val])

    const getDefaultFieldName = (fieldNum: number) => {
        if (!data) return ""
        if (!data.fields[fieldNum]) return ""
        return data.fields[fieldNum].name
    }
    ...
    return <KartAIBox>
        ...
        >
            ...
            <KartAITextField defaultValue={data?.cardType.name} id="card-type-name" size="medium"
                             label={StaticText.NAME} variant="outlined" fullWidth/>
            ...
            <KartAIBox mt={2}>
                {numberArray(0, controller.states.numOfFieldsState.val - 1).map(fieldNum => {
                    return <KartAITextField defaultValue={getDefaultFieldName(fieldNum)} key={fieldNum} mb={1.2}
                                            size="medium" id={`field-name-${fieldNum}`}
